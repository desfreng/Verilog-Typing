...
This module formalizes the specification for SystemVerilog expressions bit sizing. *)
Module Spec.
  Section SpecDef.

    (**md ## Self-Determined Size Computation - `determine`
    ...
    (**md ## Context-Determined Size Propagation - `propagate`
    ...
    (**md `propagate_def` aggregates all constraints into a single specification
  ...
  (**md `prop_split`: Decomposes a hypothesis of the form `propagate_def e f`
  ...
  (**md `prop_gen_eq`: Specializes the appropriate propagation hypothesis
  ...
  (**md `cure_propagate e f p`: Wraps a propagation function `f` to ensure
  ...
  Theorem propagate_val :
    forall e f, propagate_def e f -> forall p, IsPath e p -> exists n, f p = n.
  Proof.
