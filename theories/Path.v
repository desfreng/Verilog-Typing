...
Module Path.
  ...
  (**md `removelast_length`: Removing the last element of a non-empty list strictly
     reduces its length. *)
  Lemma removelast_length :
    forall T (l: list T), l <> [] -> length (removelast l) < length l.
  Proof.
    induction l; intros.
    - contradiction.
  ...
  (**md `path_sep_Some`: When `path_sep l = Some (l', x)`,
     the list `l` equals `l' ++ [x]`, and the length of `l'` is strictly less than that
     of `l`. *)
  ...
  (**md ## Induction Principle on Paths
  ...
  Theorem path_ind :
    forall (P : path -> Prop), P [] -> (forall x l, P l -> P (l ++ [x])) -> forall p, P p.
  Proof.
