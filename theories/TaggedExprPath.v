...
Module TaggedExprPath.
  (**md `IsTypedPath e p` states that `p` is a valid navigation path in the
     tagged expression `e`.
  ...
  (**md `sub_typed_expr e p` computes the tagged subexpression reached by `p`.
  ...
  (**md Valid paths are always realizable as successful lookups. *)
  ...
  (**md Successful lookups always induce valid typed paths. *)
  ...
  (**md Path validity is equivalent to lookup success. *)
  ...
  (**md Subexpression lookup composes over path concatenation.
  ...
  (**md Path validity also composes over path concatenation.
  ...
  (**md Decidability of typed paths. *)
