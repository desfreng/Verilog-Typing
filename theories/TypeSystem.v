...
(**md # Typing system definition

In this module, we define our typing system and prove some of its properties. *)
Module TypeSystem.

  (**md ## Resizable Expressions
     This predicate characterizes the expressions that can be resized atomically,
     i.e., by changing only the root width annotation while leaving child widths
     unchanged. In the paper, this corresponds to the class of
     "atomically resizable expressions" used by the `Resize<==` rule. *)
  ...
  (**md ## Path-Function Combinators
     Typing judgments return a sizing function `f : path -> option nat` recording
     final widths of all valid subexpressions. These combinators correspond to the
     function operators introduced in the paper (Figure "function-operators"). *)

  (**md Build a singleton sizing function for a leaf: only the root path has a width. *)
  ...
  (**md Replace the root width while preserving all strict subpaths.
     This is the function-level counterpart of atomic resizing. *)
  ...
  (**md Combine two child sizing functions under a binary node.
     Child `0` is the left operand, child `1` is the right operand. *)
  ...
  (**md Combine one child sizing function under a unary node.
     Only child `0` is valid below the root. *)
  ...
  (**md Combine three child sizing functions under a conditional node.
     Child order is condition (`0`), true branch (`1`), false branch (`2`). *)
  ...
  (**md Combine a list of child sizing functions under concatenation.
     Index `i` selects the `i`th argument. *)
  ...
  (**md ## Bidirectional Judgments

     `synth` computes a self-determined width together with a full sizing function.
     `check` verifies an expression against a required width and records the
     resulting sizing function.

     These judgments implement the paper's core pair:
     - synthesis: `e ==> n` (written here `e ==> n -| f`),
     - checking: `e <== n` (written here `e <== n -| f`). *)
  ...
  (**md Utility tactic that inverts synthesis/checking derivations and
     `Resizable` witnesses. Used throughout structural proofs on derivations. *)
  Ltac inv_ts :=
    match goal with
  ...
  (**md Root soundness for synthesis: the root path `[]` always maps to the
     synthesized width. *)
  ...
  (**md Root soundness for checking: the root path `[]` always maps to the
     checked width. *)
  ...
  (**md Induction hypothesis schema for concatenation arguments.

     It states per-argument determinism (same synthesized width and function for
     two synthesis derivations of the same child). *)
  ...
  (**md Determinism of synthesized root width for concatenation, assuming
     per-argument determinism. *)
  ...
  (**md Determinism of synthesized sizing function for concatenation,
     assuming per-argument determinism. *)
  ...
  Proof.
    ...
    inv H. inv H0. intros.
    assert (Heqf:
             forall n f1 f2, nth_error fs n = Some f1 -> nth_error fs0 n = Some f2 ->
                             forall p, f1 p = f2 p).
  ...
  (**md Pointwise monotonicity for concatenation follows from function
     determinism in this setting. *)
  ...
  (**md Checking is upward closed.

     If an expression checks at width `t`, it also checks at any larger width.
     This is the operational form of "expressions can grow" used in the
     synthesis/checking correspondence proof. *)
  Theorem check_can_grow :
    forall e t f, e <== t -| f -> forall s, t <= s -> exists f', e <== s -| f'.
  Proof.
  ...
  (**md Master structural lemma collecting key metatheory for one expression:
     - synthesis/checking order (`t <= s`),
     - synthesis determinism on widths,
     - determinism of sizing functions (synth/synth, check/check, synth/check),
     - pointwise monotonicity of sizing functions.

     In the paper, these components support the determinism and
     synthesis-checking correspondence results (Lemma 2 and Lemma 3). *)
  Lemma check_grow_synth_and_synth_inj :
    ...
    (forall t f1 f2, e <== t -| f1 -> e <== t -| f2 -> forall p, f1 p = f2 p) /\
    (forall t f1 f2, e ==> t -| f1 -> e <== t -| f2 -> forall p, f1 p = f2 p) /\
    (forall t s f1 f2, e ==> t -| f1 -> e <== s -| f2 ->
                       forall p, le_option_nat (f1 p) (f2 p)) /\
    (forall t s f1 f2, e <== t -| f1 -> e <== s -| f2 -> t <= s ->
                       forall p, le_option_nat (f1 p) (f2 p)).
  Proof.
  ...
  (**md If `e` synthesizes to `t` and checks at `s`, then `t <= s`.
     This is one direction of the paper's Synthesis-Checking Correspondence (Lemma 3). *)
  Theorem synth_check_order :
    forall e t s f1 f2, e ==> t -| f1 -> e <== s -| f2 -> t <= s.
  Proof.
    intros. destruct (check_grow_synth_and_synth_inj e). apply (H1 _ _ _ _ H H0).
  Qed.

  (**md Determinism of synthesized width.
     Corresponds to part (1) of the paper's Derivation Determinism (Lemma 2). *)
  Theorem synth_inj :
    forall e t1 t2 f1 f2, e ==> t1 -| f1 -> e ==> t2 -| f2 -> t1 = t2.
  Proof.
    intros. destruct (check_grow_synth_and_synth_inj e).
    repeat splitHyp. apply (H2 _ _ _ _ H H0).
  Qed.

  (**md Determinism of synthesized sizing function (up to extensional equality). *)
  Theorem synth_inj_f :
    forall e t1 t2 f1 f2, e ==> t1 -| f1 -> e ==> t2 -| f2 -> forall p, f1 p = f2 p.
  Proof.
    intros. destruct (check_grow_synth_and_synth_inj e).
    repeat splitHyp. rewrite (H2 _ _ _ _ H H0) in H. apply (H3 _ _ _ H H0).
  Qed.

  (**md Agreement of synthesis and checking functions when target widths are equal. *)
  Theorem synth_check_inj_f :
    forall e t f1 f2, e ==> t -| f1 -> e <== t -| f2 -> forall p, f1 p = f2 p.
  Proof.
    intros. destruct (check_grow_synth_and_synth_inj e). firstorder.
  Qed.

  (**md Determinism of checking sizing functions for a fixed target width.
     This corresponds to part (2) of paper Derivation Determinism (Lemma 2). *)
  Theorem check_inj_f :
    forall e t f1 f2, e <== t -| f1 -> e <== t -| f2 -> forall p, f1 p = f2 p.
  Proof.
    intros. destruct (check_grow_synth_and_synth_inj e). firstorder.
  ...
  (**md Pointwise monotonicity from synthesis to checking:
     values recorded in synthesized paths do not exceed checked values. *)
  ...
  (**md Pointwise monotonicity between two checking derivations, assuming root
     width order. *)
  ...
  (**md From synthesis to checking at any larger width.

     Together with `synth_check_order`, this yields the iff in theorem
     `synth_check` below (paper Synthesis-Checking Correspondence, Lemma 3). *)
  Lemma synth_can_check :
    forall e t s f, e ==> t -| f -> t <= s -> exists g, e <== s -| g.
  Proof.
  ...
  (**md Synthesis-checking correspondence:
     an expression checks at width `s` iff `s` is at least its synthesized width.

     This is the formal counterpart of the paper's Synthesis-Checking
     Correspondence (Lemma 3). *)
  Theorem synth_check :
    forall e t f, e ==> t -| f -> forall s, t <= s <-> exists g, e <== s -| g.
  Proof.
    split; intros.
  ...
  (**md Structural characterization of domains of sizing functions.

     For both synth and check, a path is valid in the expression iff the produced
     function is defined at that path. *)
  ...
  (**md Specialization of `func_on_path` to synthesis derivations. *)
  Theorem synth_f_path :
    forall e t f, e ==> t -| f -> forall p, IsPath e p <-> exists n, f p = Some n.
  Proof.
    intros e. destruct (func_on_path e). assumption.
  Qed.


  (**md Specialization of `func_on_path` to checking derivations. *)
  Theorem check_f_path :
    forall e t f, e <== t -| f -> forall p, IsPath e p <-> exists n, f p = Some n.
  Proof.
    intros e. destruct (func_on_path e). assumption.
  Qed.

  (**md Subexpression decomposition of sizing functions.

     For any successful subexpression lookup `e @[p] = Some e'`, the derivation on
     `e` induces a derivation on `e'` whose function agrees with the parent
     function on all suffixes `p ++ k`.

     This is the technical core behind the paper's sub-expression correspondence
     lemmas. *)
  Lemma f_sub_exp : forall e,
    ...
    /\ (forall t f,
          ...
          e @[p] = Some e' ->
          exists t' f',
            (e' ==> t' -| f' \/ e' <== t' -| f') /\ forall k, f (p ++ k) = f' k).
  Proof.
    Ltac _inv_se_f :=
      match goal with
      | [ H: (_ _) @[ _ ] = _ |- _ ] => inv H
  ...
  (**md Synthesis-side sub-expression correspondence.
     Matches the paper's Sub-expression Synthesis result (Lemma 4). *)
  ...
  (**md Checking-side sub-expression correspondence.
     Matches the paper's Sub-expression Checking result (Lemma 5). *)
  ...
  (**md Totality of synthesis:

     Every expression admits a synthesis derivation.
     This corresponds to part (1) of the paper's Universal Derivations (Lemma 1). *)
  ...
  (**md Totality of checking:
     Every expression checks against some width.

     This corresponds to part (2) of the paper's Universal Derivations(Lemma 1). *)
  ...
  (**md Auxiliary predicate used to state minimality:
     `P e s` means `e` checks at every width `t >= s`. *)
  Definition P e s := forall t, s <= t -> exists g, e <== t -| g.

  (**md Minimality principle for synthesized widths.
     If `s` is the least lower bound of widths allowing checking, then `e`
     synthesizes at width `s`.

     This formalizes the paper's minimality statement
     (Theorem 1, "Synthesis as Minimal Width"). *)
  Theorem synth_minimal_check :
    forall e s, P e s -> (forall u, P e u -> s <= u) -> exists f, e ==> s -| f.
  Proof.
