...
Module Algo.
  (**md Bottom-up computation of self-determined width.
  ...
  (**md Top-down propagation of final widths.
  ...
  (**md Complete two-phase algorithm for typing an expression.
  ...
  (**md Extract the final width annotation at a given path from a tagged expression.
  ...
  (**md Shape preservation (forward direction).
  ...
  (**md Shape preservation (backward direction).
  ...
  (**md Path equivalence for propagation.
     A path is valid in `e` iff it is valid in every `propagate e s`. *)
  Theorem propagate_shape :
    forall e p, IsPath e p <-> forall s, IsTypedPath (propagate e s) p.
  Proof.
    split; intros.
  ...
  (**md Path equivalence for the full algorithm. *)
  ...
  (**md Determine compatibility with the specification.
  ...
  (**md Root tag invariant of propagation.
  ...
  (**md Root width computed by `type`.
  ...
  (**md Small inversion tactic for path hypotheses.
  ...
  (**md Main pointwise correspondence lemma.
  ...
  Lemma propagate_size_path_lemma : forall e s fc,
    e <== s -| fc ->
  ...
  (**md Global checking result for propagation.
  ...
  (**md End-to-end correctness of the exported algorithm.
