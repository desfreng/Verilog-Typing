...
(**md Module imported from coq-tricks.
Provides a mechanism to store learnt hypotheses to avoid re-deriving
the same facts repeatedly during proof automation. *)
Module Learn.
  (**md A wrapper type used to record that a proposition `P` has been learnt. *)
  Inductive Learnt {P:Prop} :=
  | AlreadyLearnt (H:P).

  (**md Internal tactic: marks a hypothesis as learnt if it hasn't been recorded yet. *)
...
Module Utils.
  ...
  (**md `inv H`: performs inversion on hypothesis `H`, clears it, and substitutes
  resulting equalities into the context. *)
  Ltac inv H := inversion H; clear H; subst.

  (**md `antisym`: applies antisymmetry on a pair of `<=` relations in the context
  ...
  (**md `splitHyp`: destructs a conjunction in the context. *)
  Ltac splitHyp :=
    match goal with
    | [ H: _ /\ _ |- _ ] => destruct H
    end.
  
  (**md `existsHyp`: destructs an existential hypothesis in the context. *)
  Ltac existsHyp :=
    match goal with
    | [ H: ex _ |- _ ] => destruct H
    end.

  (**md Lemma and tactic to help build conjunctions incrementally. *)
  Lemma and_with_impl : forall P Q, P -> (P -> Q) -> P /\ Q.
  Proof.
    split; intros; auto.
  Qed.

  (**md `splitAnd`: prepares the goal for proving a conjunction by introducing
  ...
  (**md Decomposes the `max` function into explicit cases, preserving order relations. *)
  ...
  (**md Auxiliary tactic used by `splitMax` to destruct results of `max_dec_bis`. *)
  Ltac _useMaxDecBis :=
    match goal with
    | [ H: (_ = _ /\ _) \/ (_ = _ /\ _) |- _ ] =>
  ...
  (**md `splitMax`: applies `max_dec_bis` to all occurrences of `max x y` in either
  ...
  (**md Lemma stating that if two lists have the same length, then for every element
  ...
  (**md `gen_nth`: given two lists of equal length and an index known to exist in one,
  ...
  (**md ## Indexed mapping function *)

  (**md `mapI'` applies a function `f` to each element of a list `l`,
  ...
  (**md `mapI` is a specialization of `mapI'` starting from index 0. *)
  Definition mapI {X Y: Type} (f: nat -> X -> Y) := mapI' f 0.

  (**md Characterization of elements in `mapI'`:
  ...
  (**md Simplified version of `mapI'_values` for the common case where indexing starts at 0. *)
  ...
  (**md ## Custom helper functions *)

  (**md Computes the sum of a list of natural numbers. *)
  ...
  (**md Defines an ordering relation on optional natural numbers,
