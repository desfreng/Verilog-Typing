...
(**md This module defines the untyped abstract syntax tree (AST) of SystemVerilog
   expressions. *)
Module Expr.
  Unset Elimination Schemes.

  (**md ## SystemVerilog Expression Model
  ...
  Inductive Expr : Type :=
  | EOperand (size: nat)
  (**md Atomic operand, storing its bitwidth `size` (as determined by the $\Gamma$
     function). *)
  | EBinOp (lhs rhs: Expr)
  (**md Binary operator expression. *)
  | EUnOp (arg: Expr)
  (**md Unary operator expression. *)
  | EComp (lhs rhs: Expr)
  (**md Comparison expression (e.g., equality, inequality, relational ops). *)
  | ELogic (lhs rhs: Expr)
  (**md Logical expression (e.g., logical AND, OR). *)
  | EReduction (arg: Expr)
  (**md Reduction expression (e.g., bitwise reduction operators). *)
  | EShift (lhs rhs: Expr)
  (**md Bitwise shift operation (e.g., left/right shift). *)
  | EAssign (lval: nat) (arg: Expr)
  (**md Assignment expression, storing the size of the left-hand value `lval`. *)
  | ECond (cond tb fb: Expr)
  (**md Conditional (ternary) expression: `cond ? tb : fb`. *)
  | EConcat (args: list Expr)
  (**md Concatenation of multiple subexpressions. *)
  | ERepl (amount: nat) (arg: Expr)
  (**md Replication operator: repeats `arg` `amount` times. *)
  .
  ...
  (**md ## Custom Induction Principle for `Expr`
  ...
  Section Expr_rect.
    ...
    (**md Recursive definition of `Expr_rect`, extending the induction principle to handle
  ...
  (**md Convenient versions of `Expr_rect` for `Prop` and `Set`. *)
