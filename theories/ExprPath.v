...
Module ExprPath.
    ...
    (**md `IsPath e p` expresses that `p` is a valid path within the expression `e`.
  ...
  (**md `sub_expr e p` navigates the expression `e` according to path `p`
  ...
  (**md `sub_expr_valid`: if `sub_expr e p = Some f`, then `p` is a valid path of `e`
     leading to `f`. *)
  ...
  (**md `IsPath_sub_expr_iff`: a path is valid for an expression if and only if
  ...
  (**md `sub_expr_nil`: the empty path always corresponds to the expression itself. *)
  Lemma sub_expr_nil: forall e, e @[[]] = Some e.
  Proof.
    intros. destruct e; reflexivity.
  Qed.

  (**md `sub_expr_chunk`: subexpression lookups compose under path concatenation.
  ...
  (**md `IsPath_chunk`: decomposes the validity of a concatenated path `p ++ c`
  ...
  (**md `IsPath_dec`: the predicate `IsPath e p` is decidable - every path is either
