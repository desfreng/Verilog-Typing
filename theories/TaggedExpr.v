...
(**md # TaggedExpr Module

This module mirrors the untyped expression AST from `Expr`, but enriches every
node with an external annotation (a *tag* ). In this project, tags are used to
store computed widths during the Determine/Propagate algorithm. *)
Module TaggedExpr.
  (**md ## Tagged Expression Syntax

     The syntax is factored into:
     - `TaggedExprKind T`: the operator shape and recursive children,
     - `TaggedExpr T`: a node pairing one `TaggedExprKind` with a tag value. *)
  Section TaggedExpr_def.
    Variable T: Type.

    Unset Elimination Schemes.

    (**md Core expression constructors with the same shape as `Expr`, but with
       recursive fields of type `TaggedExpr`.

       The constructor `TOperand` and assignment field `lval` keep source sizes,
       while all computed metadata lives in the outer `TExpr ... tag` wrapper. *)
    ...
    (**md List helper used by the custom induction principle for the empty
       concatenation case. *)
    Lemma HNil: forall n e, nth_error [] n = Some e -> P e.
    Proof. intros; destruct n; inv H. Qed.

    (**md List helper used by the custom induction principle for the non-empty
       concatenation case. *)
    ...
    (**md ## Custom Induction Principle for `TaggedExpr`

       This principle is analogous to `Expr.Expr_ind`, but for tagged trees.
       It provides recursive hypotheses for all direct children and, for
       concatenations, for every indexed element of the argument list. *)
  ...
  (**md Project the annotation stored at the root of a tagged expression. *)
